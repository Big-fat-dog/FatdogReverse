...
// 网络关卡 15（对应教程 20）：千数求和——数据只在本地服务端，分页加载（保留跳页）。
public class s5Activity extends Activity {
    static final String SUM_HASH = "a65383aadefe9c5d79981b694abb93110996f7c1f5352ee4b9376be27e19fdf9";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("服务端有 100 页数据、每页 10 个数字。\n分页请求每页都带签名，取回全部页面后求和提交。");
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(s5Activity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(s5Activity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new AlertDialog.Builder(s5Activity.this)
                        .setTitle("提示")
                        .setMessage("每页请求 sign = HMAC-SHA256(key, page=N&ts=T)，密钥拆在 Kx/Sg 的异或数组里。先 Frida 观察签名，或直接复刻逐页取数求和。")
        ...
        box.addView(Ui.banner(this, R.drawable.level_15, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(s5Activity.this, "FLAG_18_L15{thousand_number_sum}");
                    PassLog.mark(s5Activity.this, "L15");
                } else {
                    Toast.makeText(s5Activity.this,
    ...
    private void loadPage(final int page) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String body = Sg.fetchPage(base, page);
                    final JSONObject obj = new JSONObject(body);
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            loading = false;
                            currentPage = obj.optInt("page", page);
                            JSONArray arr = obj.optJSONArray("nums");
                            int[] nums = new int[arr.length()];
                            for (int i = 0; i < arr.length(); i++) nums[i] = arr.optInt(i);
                            render(nums);
                            renderNav();
                            status.setText("已加载第 " + currentPage + " / " + PAGES + " 页，本页 " + nums.length + " 个数字");
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            loading = false;
                            status.setText("请求失败: " + e.getMessage() + "（可重试）");
                        }
                    });
                }
            }
    ...
    private String baseUrl() {
        try {
            ...
            return NetHost.resolve(cfg.getJSONObject("server").getString("api_base_url"), false);
        } catch (Exception e) {
            return Sg.BASE;
        }
    }
