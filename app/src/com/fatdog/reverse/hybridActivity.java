...
// 天地秘境·须弥界 KL45「深渊合璧」：收官卷，把前四关手段并成两道锁。
// 第一层：页面脚本用分组密码把明文参数锁成密文 enc（密钥藏在混淆的 JS 里）；
...
public class hybridActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("深渊底下两道锁，是这一路的合璧。\n"
                + "页面先用分组密码把参数锁死，native 再压一道摘要签——\n"
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(hybridActivity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(hybridActivity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new AlertDialog.Builder(hybridActivity.this)
                        .setTitle("提示")
                        .setMessage("收官卷：两道锁，两把钥，两道算法。\n\n"
                                + "第一层在页面脚本里：它用分组密码把明文参数锁成一段 hex 密文，\n"
                                + "密钥藏在被搅乱的那堆代码里（原文里看不见，得自己解）。\n"
                                + "第二层在 native：它对「钥 + 明文参数 + 那段密文」取一道摘要签，格式与钥都得还原。\n\n"
        ...
        box.addView(Ui.banner(this, R.drawable.level_kl45, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(hybridActivity.this, "FLAG_19_KL45{abyssal_union}");
                    PassLog.mark(hybridActivity.this, "KL45");
                } else {
                    Toast.makeText(hybridActivity.this,
        ...
        refreshGuard();
        web.loadUrl("file:///android_asset/h5/hybrid_kl45.html");
    }

    private void refreshGuard() {
        try {
            guard.setText("反调试自检：" + HybridCore.nativeStatus());
        } catch (Throwable t) {
    ...
    private void loadPage(final int page) {
        ...
        web.evaluateJavascript("(window.fdPack)(" + page + "," + ts + ")",
                new ValueCallback<String>() {
                    ...
                    public void onReceiveValue(String value) {
                        ...
                        // 第二层：交给 native 做普通 MD5 摘要签名
                        String sign;
                        try {
                            sign = HybridCore.nativeFullSign(page, ts, enc);
                        } catch (Throwable t) {
                            loading = false;
                            status.setText("native 签名失败：" + t.getMessage());
                            return;
                        }
                        HybridCore.fetch(base, page, ts, enc, sign, new HybridCore.Cb() {
                            @Override
