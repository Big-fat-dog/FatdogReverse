...
// 天地秘境·太玄之初 KL19「虚空造化」的 JNI 桥 + 网络助手（二代综合：指令抽取 + 反调试 + 自校验）。
// libbison.so 把标记逐字节抽走、缓冲区先填 nop，走还原点才逐条填回；还原点外面裹着反调试哨兵
// （ptrace/TracerPid/27042 端口/maps 指纹/CRC 自校验，命中 ≥2 项即抹掉还原结果）。
public class Gk {
    static {
        System.loadLibrary("bison");
    }
    ...
    static void fetchPage(String base, final int page, final Cb cb) {
        ...
        final String sign = Gk.nativeSign(page, ts);
        try {
            final OkHttpClient c = trustClient();
            String url = base + "/api/kl19?page=" + page + "&ts=" + ts
                    + "&sign=" + sign;
