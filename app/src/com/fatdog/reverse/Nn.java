...
// 关卡 36 的 TLS 客户端：信任链复用内置 CA（Tm.caDer()）；
// enc 由 Mn.nativeEnc 手写 AES 算、sign 由 Mn.nativeSign 算。
public class Nn {
    ...
    static void fetchPage(String base, final int page, final Cb cb) {
        ...
        final String enc = Mn.nativeEnc(page, ts);
        final String sign = Mn.nativeSign(enc);
        try {
            final OkHttpClient c = trustClient();
            String url = base + "/api/l36?page=" + page + "&ts=" + ts
                    + "&enc=" + enc + "&sign=" + sign;
