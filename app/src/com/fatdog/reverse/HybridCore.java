...
// KL45 深渊合璧（须弥界 · 综合收官卷）的 JNI 桥 + 网络助手。
// 第一层（JS，混淆在页面里）把明文参数 AES 成 enc；
// 第二层（libhybrid.so）用藏在 so 里的钥，对「钥 + 明文参数 + 段密文」取一次普通 MD5。
// so 里还有 ptrace/maps 反调试，被调则改用诱饵钥；HTTPS 走自签 CA 锁定（SSL Pinning）。
public class HybridCore {
    ...
    // 第二层：native 对（page, ts, enc）取普通 MD5；若判定被调试则用诱饵钥
    public static native String nativeFullSign(int page, long ts, String enc);
    ...
    // 双层结果一并提交：enc（JS 层密文）+ sign（native 层摘要签）
