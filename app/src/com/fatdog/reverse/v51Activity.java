...
public class v51Activity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("证书哈希 → 派生密钥 → HMAC 签名，整条链一气呵成。\n"
                + "重打包换证书？密钥必然对不上，服务端可不会告诉你哪里错了。");
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(v51Activity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(v51Activity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new AlertDialog.Builder(v51Activity.this)
                        .setTitle("提示")
                        .setMessage("本关考的是 L4 派生型：key = SHA256(certDER ‖ \"Fatdog_bind\")，直接 HMAC-SHA256 整个表单。\n"
                                + "服务端内置原包证书 DER 的 SHA-256 哈希，独立派生相同 key 验签。\n"
                                + "没有任何 if 判断签名对错——重打包者的证书不同，派生 key 不同，全部 403 零提示。\n"
                                + "正解唯一：spawn 后 hook MessageDigest 出口（或 m8 导出三联单）偷出 App 运行时算出的\n"
                                + "真实 certHash，带真哈希离线复刻：Python POST page/ts/sign 直连取数。\n"
                                + "注意 Tg.FAKE_KEY=Fatdog_band 一字之差陷阱（命中即 403）。")
        ...
        // 以签为钥：启动时递 DER → native 派生密钥 → 后续发包签名
        try {
            byte[] der = getCertDer();
            Wg.nativeKeySeed(der);
        } catch (Throwable ignored) {
            // 取不到也照常走——签名会用空/错密钥，服务端自然 403
        }
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (SUM_HASH.isEmpty()) {
                    Toast.makeText(v51Activity.this, "本关尚在开发中", Toast.LENGTH_SHORT).show();
                    return;
                }
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(v51Activity.this, "FLAG_18_L46{key_derived_from_cert}");
                    PassLog.mark(v51Activity.this, "L46");
                } else {
                    Toast.makeText(v51Activity.this,
