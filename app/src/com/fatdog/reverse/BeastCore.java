...
// 天地秘境·迷阵 KL54「困兽犹斗」的 JNI 桥 + 网络助手（间接跳转 + 魔改 SM4 + 魔改 Base64）。
// libbeast.so 用函数指针表间接派发签名（4 个同形副本，只有一个是真签名），
// 魔改 SM4 加密、魔改 Base64 藏钥。
// 请求链：enc = hex(魔改SM4-ECB(sm4_key, "page=N&ts=T")), sign = SHA256(标记|page|ts)。
public class BeastCore {
    static {
    ...
    static void fetchPage(String base, final int page, final Cb cb) {
        ...
        final String enc = BeastCore.nativeEnc(page, ts);
        final String sign = BeastCore.nativeSign(page, ts);
        try {
            final OkHttpClient c = trustClient();
            String url = base + "/api/kl54?page=" + page + "&ts=" + ts
                    + "&enc=" + enc + "&sign=" + sign;
