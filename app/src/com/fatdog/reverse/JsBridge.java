...
// KL43 桥上听风（须弥界 · JSBridge 协议逆向 + JS 侧消息签名）的 JNI 桥 + 网络助手。
// libjsbridge.so 是协议分发器：持有一张 cmd→handler 表，按表分发 bridge 消息。
// 而消息的 sign 不在 native——它在混淆后的前端脚本里算，本类只负责把它带上去发请求。
public class JsBridge {
    static {
        System.loadLibrary("jsbridge");
    }

    private JsBridge() {}

    // 协议 dispatch 表：cmd → handler
    public static native String nativeGetDispatchTable();

    // 按 dispatch 表分发一条 bridge 消息，返回 handler 名（认不出则 "unknown"）
    public static native String nativeHandle(String msg);
    ...
    // 以 bridge 消息（cmd/page/ts/sign）为表单提交查询
    static void query(String base, int page, long ts, String sign, final Cb cb) {
        try {
