...
// 天地秘境·太玄之初 KL19「虚空造化」：二代综合——指令抽取 + 反调试 + SO 自校验。
// 在 KL18 方法抽取之上，还原点外面裹了一层哨兵：判定被调试/被篡改，就把还原出的标记抹掉。
// 取数链路仍是网络求和：Gk 用 HMAC-SHA256（钥匙由标记派生）给 "page=N&ts=T" 签名，
// 带签请求 /api/kl19 取数后求和。
public class emberActivity extends Activity {
    static final String SUM_HASH = "1e1eae8b16ff41d6ca9a0713b16abc7dcdb6115080e162074dce1c4f6e241975";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("这一回，抽空的指令外面还站着一个哨兵。\n"
                + "它一旦觉得有人在盯着，就把刚填回去的东西抹掉。");
        tv.setGravity(Gravity.CENTER);
        box.addView(tv, Ui.wrap(4));

        // 只读自检：只显示哨兵状态，不还原、不判胜、不吐标记
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(emberActivity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(emberActivity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new AlertDialog.Builder(emberActivity.this)
                        .setTitle("提示")
                        .setMessage("这一关把抽空和还原都藏进了受保护的地方，"
                                + "外面还立着哨兵：它不只盯一处，而是看好几个地方——"
                                + "有没有人附着上来、端口通不通、内存里有没有熟面孔，"
                                + "连自己那张表动没动过都要验一遍。\n\n"
                                + "哨兵不是一惊一乍的性子，单项可疑它还不动手；"
                                + "但凑够两样，它就把刚填回去的东西抹平。\n\n"
                                + "抹平之后发出的请求当然也就不再作数。"
        ...
        box.addView(Ui.banner(this, R.drawable.level_kl19, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(emberActivity.this, "FLAG_18_KL19{debug_beaten}");
                    PassLog.mark(emberActivity.this, "KL19");
                } else {
                    Toast.makeText(emberActivity.this,
    ...
    private void refreshGuard() {
        try {
            guard.setText("哨兵自检：" + Gk.nativeStatus());
        } catch (Throwable t) {
            guard.setText("哨兵自检：不可用");
        }
    }
    ...
    private void loadPage(final int page) {
        ...
        Gk.fetchPage(base, page, new Gk.Cb() {
            @Override
