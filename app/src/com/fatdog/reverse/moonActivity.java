...
// 碧落天 KL39「月下独酌」：dart:ffi 双向往调 + 密钥分片。
// 取数链路是网络求和：摘要先由前端侧记下，再由 native 侧用两瓣拼回的密钥封住，
// 服务端解出正确的摘要才放数。
public class moonActivity extends Activity {
    static final String SUM_HASH = "0e84adc50365026b35f3595727df3d7ab5833873247dc9c639675259e08e4eb5";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("月下的酒只剩半盏，另一半搁在别处。\n"
                + "两瓣合上，月光才认得这一页；\n"
                + "摘要先记下，密文随后动身。");
        tv.setGravity(Gravity.CENTER);
        box.addView(tv, Ui.wrap(4));

        // 只读自检：只报密码原语与两瓣来源，不吐密钥、不判胜
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(moonActivity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(moonActivity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new AlertDialog.Builder(moonActivity.this)
                        .setTitle("提示")
                        .setMessage("酒分两处：一半落在随信那卷东西里，另一半沉在这边的井底。\n"
                                + "只看一边，凑不出完整的一盏——得把两瓣接起来，才是那把匙。\n\n"
                                + "封之前，先把这一页的名字与时刻记成一句摘要（那段在另一侧算），\n"
                                + "再用合起来的匙把它封住。两边都看清了，数字才肯出来。")
        ...
        box.addView(Ui.banner(this, R.drawable.level_kl39, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    ...
                    Toast.makeText(moonActivity.this,
    ...
    private void refreshGuard() {
        try {
            guard.setText("算法自检：" + FlutterFFI.nativeGetStatus());
        } catch (Throwable t) {
    ...
    private void loadPage(final int page) {
        ...
        FlutterFFI.fetchPage(base, page, new FlutterFFI.Cb() {
            @Override
