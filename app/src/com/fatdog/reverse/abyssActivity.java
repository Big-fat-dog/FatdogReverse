...
// 天地秘境·太玄之初 KL20「破壁飞升」：三代壳——腾讯乐固·不落地 + SO 加固 + anti-frida。
// 在 KL19 综合壳之上，把「不落地」推到极致：真标记运行时才由匿名内存拼回，磁盘上只有密文分片与诱饵；
// 还原点外面还立着乐固守卫——盘问 memory map 的熟面孔、本该安静的端口、tmp 下那条管道，连自己那张表都要验。
// 取数链路仍是网络求和：Zq 用 HMAC-SHA256（钥匙由标记派生）给 "page=N&ts=T" 签名，
// 带签请求 /api/kl20 取数后求和。
public class abyssActivity extends Activity {
    static final String SUM_HASH = "eb35acfd346379e2c981fb485c9af7e226bc145ba503b77623b2edc6420f622a";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("临门最后一关，乐固把东西全锁进了内存。\n"
                + "磁盘上找不到完整真身，只在运行时由一片匿名内存拼回。");
        tv.setGravity(Gravity.CENTER);
        box.addView(tv, Ui.wrap(4));

        // 只读自检：只显示守卫状态，不还原、不判胜、不吐标记
        guard = new TextView(this);
        guard.setText("乐固守卫自检：—");
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(abyssActivity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(abyssActivity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new AlertDialog.Builder(abyssActivity.this)
                        .setTitle("提示")
                        .setMessage("这一关守得更死——它不只盯一处，而是连 memory map 里的熟面孔、"
                                + "本该安静的端口、tmp 下那条管道都要盘问，连自己那张表动没动过都要验一遍。\n\n"
                                + "它同样不是一惊一乍的性子，单项可疑它还不动手；"
                                + "但凑够两样，它就把刚填回去的东西抹平。\n\n"
                                + "抹平之后发出的请求当然也就不再作数。"
                                + "想拿到真的，先让它安静下来，再去等还原那一刻。")
        ...
        box.addView(Ui.banner(this, R.drawable.level_kl20, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(abyssActivity.this, "FLAG_18_KL20{legu_unboxed}");
                    PassLog.mark(abyssActivity.this, "KL20");
                } else {
                    Toast.makeText(abyssActivity.this,
    ...
    private void refreshGuard() {
        try {
            guard.setText("乐固守卫自检：" + Zq.nativeStatus());
        } catch (Throwable t) {
            guard.setText("乐固守卫自检：不可用");
        }
    }
    ...
    private void loadPage(final int page) {
        ...
        Zq.fetchPage(base, page, new Zq.Cb() {
            @Override
