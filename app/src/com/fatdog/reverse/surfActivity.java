...
// 天地秘境·须弥界 KL41「浅滩拾贝」：H5 壳——WebView 承载一页 H5，native 侧用 addJavascriptInterface
// 注入一座「桥」。签从桥上来，桥向 native 要；桥的名字与它暴露的方法，就刻在壳的缝里。
// 真钥匙不在 Java/DEX，藏在 libh5shell.so 的异或数组深处，发包那一刻才由桥取出、拼出、签上。
// 取数链路仍是网络求和：H5Shell 用 HMAC-SHA256 给 "page=N&ts=T" 签名，带签请求 /api/kl41 取数后求和。
public class surfActivity extends Activity {
    static final String SUM_HASH = "f95bb6eaf9efb42ea33cc266564a90c4a976fa64a8ff33dbeb89320768824475";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("浅滩上搁着一只半开的贝壳，里头嵌着一页 H5。\n"
                + "页不自签——它搭一座桥，向 native 借。\n"
                + "桥叫什么、桥上递什么，都藏在壳缝里。");
        tv.setGravity(Gravity.CENTER);
        box.addView(tv, Ui.wrap(4));

        // H5 壳：WebView 承载本地 H5，native 注入 bridge 对象
        web = new WebView(this);
        WebSettings ws = web.getSettings();
        ws.setJavaScriptEnabled(true);
        ws.setDomStorageEnabled(true);
        web.addJavascriptInterface(new ShellBridge(), "FatdogBridge");
        LinearLayout.LayoutParams wlp = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, Ui.dp(190));
        ...
        reload.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                web.loadUrl("file:///android_asset/h5/index_kl41.html");
            }
        });
        ...
        jump.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    else Toast.makeText(surfActivity.this, "页码超出范围 1-" + PAGES, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(surfActivity.this, "请输入页码", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        hint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new AlertDialog.Builder(surfActivity.this)
                        .setTitle("提示")
                        .setMessage("这一页的签不是前端自己算的——它搭了一座桥，伸手向 native 要。\n\n"
                                + "在反编译工程里找 WebView 注入的那个对象，看它暴露了哪些方法。\n"
                                + "可真钥匙并不在 Java 里；顺着桥往 so 里走，才能摸到那张异或数组。\n\n"
                                + "盘上留了两个标记，只差几个字母，一个通、一个空——仔细比对。")
        ...
        box.addView(Ui.banner(this, R.drawable.level_kl41, 150));
        ...
        subBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sha256Hex(ans).equals(SUM_HASH)) {
                    Celebration.show(surfActivity.this, "FLAG_19_KL41{shallow_shell_found}");
                    PassLog.mark(surfActivity.this, "KL41");
                } else {
                    Toast.makeText(surfActivity.this,
    ...
    // native 注入 WebView 的 bridge 对象：@JavascriptInterface 标注的方法即可被 H5 调用。
    // 具名内部类，Frida 可直接 Java.use("com.fatdog.reverse.surfActivity$ShellBridge")。
    public class ShellBridge {
        @JavascriptInterface
        public String getToken() {
            return H5Shell.nativeToken();
        }

        @JavascriptInterface
        public String sign(int page, long ts) {
            return H5Shell.nativeSign(page, ts);
        }
        ...
        public String methods() {
            return H5Shell.nativeGetBridgeMethods();
        }
    }
    ...
    private void loadPage(final int page) {
        ...
        H5Shell.fetchPage(base, page, new H5Shell.Cb() {
            @Override
            public void onPage(final int got, final int[] nums) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        renderNav();
                        if (nums.length == 0) {
                            status.setText("第 " + got + " 页没有拿到数字——签名大概没被认可。");
                        } else {
