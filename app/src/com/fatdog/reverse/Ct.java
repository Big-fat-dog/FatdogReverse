...
// 关卡 28 的 TLS 客户端：信任链复用内置 CA（Tm.caDer()），
// 请求签名由 Zk.nativeSign 在 libaxol.so 里算——密钥被异或藏进 .rodata，Java 层无密钥。
public class Ct {
    ...
    static void fetchPage(String base, final int page, final Cb cb) {
        ...
        final String sign = Zk.nativeSign(page, ts);   // 签名全在 native 算
        try {
            final OkHttpClient c = trustClient();
            String url = base + "/api/l28?page=" + page + "&ts=" + ts + "&sign=" + sign;
