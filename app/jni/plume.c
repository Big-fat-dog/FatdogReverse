...
/* dlsym 跨 so 调用 libm13a 和 libm13b */
static int call_m13a_nativePartA(void) {
    typedef int (*fn)(void);
    fn f = (fn)dlsym(RTLD_DEFAULT, "Java_com_fatdog_reverse_Zn_nativePartA");
    if (f) return f();
    return 0;
}

static int call_m13b_get_digest_B(char out[9]) {
    typedef int (*fn)(char*);
...
/* --- 诱饵导出 --- */
void m13c_decoy_seal(void) {}
void m13c_fold(void) {}
void m13c_spin(void) {}

/* --- JNI 桥接 --- */

/* Zn.nativeCombineFromC() → String（最终答案，交叉调用 A+B） */
JNIEXPORT jstring JNICALL
Java_com_fatdog_reverse_Zn_nativeCombineFromC(JNIEnv *env, jclass clazz) {
    (void)clazz;
    /* 交叉调用 libm13a → digest_A */
    int a = call_m13a_nativePartA();
    /* 交叉调用 libm13b → digest_B */
    char hex_b[9];
    int b = call_m13b_get_digest_B(hex_b);
