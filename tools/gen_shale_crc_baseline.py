#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""KL15 真实代码段 CRC-32 基线烘焙器（libshale.so）。

从 NDK 构建产物 app/libs/<abi>/libshale.so 里定位导出符号
Java_com_fatdog_reverse_Am_nativeGuard，取其起始
KL15_SHALE_CRC_WINDOW 字节，用标准 CRC-32（同 Python
zlib.crc32）计算基线，写入 app/jni/shale_crc_baseline.h。运行时
verify_shale_crc() 用同一算法对同一代码窗口重新算 CRC，patch 任意
窗口内指令或 inline hook 都会导致不匹配。

用法:
    python tools/gen_shale_crc_baseline.py            # 烘焙并写头文件
    python tools/gen_shale_crc_baseline.py --verify   # 校验当前 so 与头文件
    python tools/gen_shale_crc_baseline.py --dir <so目录根>  # 指定 lib 根目录
"""
import os
import re
import struct
...
HEADER = os.path.join(JNI, 'shale_crc_baseline.h')
LIBS = os.path.join(HERE, 'app', 'libs')
SYMBOL = 'Java_com_fatdog_reverse_Am_nativeGuard'
ABIS = (
    ('arm64-v8a', 'KL15_SHALE_CRC_BASELINE_ARM64'),
    ('armeabi-v7a', 'KL15_SHALE_CRC_BASELINE_ARMEABI_V7A'),
)
# nativeGuard 之后的代码段很短：窗口必须完整落在一个 PT_LOAD 内。
WINDOW = 512


def fail(msg):
    sys.stderr.write('gen_shale_crc_baseline: %s\n' % msg)
    sys.exit(1)
...
def compute_baselines(libs_root):
    result = {}
    for abi, _ in ABIS:
        path = os.path.join(libs_root, abi, 'libshale.so')
        ...
        if file_off is None:
            fail('%s: guard window crosses PT_LOAD boundary (reduce KL15_SHALE_CRC_WINDOW)' % abi)
        with open(path, 'rb') as f:
...
def write_header(baselines):
    lines = [
        '#ifndef SHALE_CRC_BASELINE_H',
        '#define SHALE_CRC_BASELINE_H',
        '',
        '/* 自动生成：tools/gen_shale_crc_baseline.py，请勿手改。 */',
        '#define KL15_SHALE_CRC_WINDOW %d' % WINDOW,
    ]
    for abi, macro in ABIS:
        lines.append('#define %s 0x%08xu' % (macro, baselines[abi]))
    lines.extend(['', '#endif /* SHALE_CRC_BASELINE_H */', ''])
    ...
    if verify:
        ...
        for abi, macro in ABIS:
            if old[macro] != baselines[abi]:
                ...
                bad = True
        if bad:
            fail('baseline does not match current libshale.so; rebuild after baking')
